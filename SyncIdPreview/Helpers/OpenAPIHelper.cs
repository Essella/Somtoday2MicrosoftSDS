...
namespace SyncIdPreview.Helpers
{
    internal class OpenAPIHelper
    {
        ...
        private readonly string schoolUUID;
        ...
        public OpenAPIHelper(string clientId, string clientSecret, string schoolUUID, SomEnvironmentConfig somConfig, IHttpClientFactory httpClientFactory, ILogger<OpenAPIHelper> logger = null)
        {
        ...
        {
            using RestClient client = new RestClient(somConfig.LoginUrl + schoolUUID);
            ...
            try
            {
                ...
                {
                    _logger?.LogWarning("Unauthorized access to Somtoday: {Response}", response.Content);
                }
            }
            catch (Exception e)
            {
                _logger?.LogError(e, "Error connecting to Somtoday API");
            }
        }

        internal async Task<List<VestigingModel>> DownloadAllInfoAsync(bool booleanFilterBylocation, string[] includedLocationCode, bool enableGuardianSync, CancellationToken cancellationToken = default)
        {
            ...
            {
                HashSet<string> includedLocations = includedLocationCode.ToHashSet(StringComparer.CurrentCultureIgnoreCase);
                vestigingen = vestigingen.Where(v => includedLocations.Contains(v.Afkorting)).ToList();
            }
        ...
        private async Task<List<Vestiging>> GetVestigingenAsync(CancellationToken cancellationToken)
        {
            ...
            {
                _logger?.LogError(e, "Error retrieving vestigingen from Somtoday");
                return vestigingen;
            }
        }
