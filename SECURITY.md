...
- Production Graph access is constrained to the Job's system-assigned managed identity. The Cloud Shell deployment script grants only `IndustryData-InboundFlow.ReadWrite.All`, `IndustryData-DataConnector.Upload`, and the validation-operation polling permission `IndustryData.ReadBasic.All`.
