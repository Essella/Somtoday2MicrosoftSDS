...
1. `Program` creates the generic host without passing command-line arguments into host configuration, then creates the logger, HTTP client factory, `FileHelper`, and `SomtodaySecretProvider`. It separately detects exact, case-insensitive `--empty-csv` arguments; all other arguments are ignored.
