...
- `SyncConfiguration` validates one inbound-flow UUID, one or more unique Somtoday institution UUIDs, credentials, location filters, username rules, guardian mode, and header-only mode.
- `OpenAPIHelper` owns Somtoday authentication and reads. It preserves cancellation and the existing bounded authentication retries.
...
2. Authenticate to Graph with `DefaultAzureCredential` and resolve the connector related to `InboundFlowId`.
...
No permanent application-owned output store exists. The only CSV persistence is the temporary SDS-owned SAS container. Multiple Container Apps Jobs may share an ACA environment, but each Job has one inbound flow and its own system-assigned identity.
